...
Single-context repo — this repo is a monorepo (`apps/web` + `apps/worker`) but shares one domain (the trivia book content pipeline: pages, books, answer keys), so one root `CONTEXT.md` covers both apps:
